...
- Discovery, recorded so later phases do not re-derive it. Canonical factual authority is `src/inventory/inventory.json`: a flat array of 11 records (10 houses, 1 villa) keyed by stable `slug`, with the same schema for both types, carrying areaSqm, sleeps, bedrooms, bathrooms, pool/poolNotes/sharedPoolWith, floors, internalStairs, view, parking, wifi, pets, hardConstraints, amenities, kitchen/kitchenNotes, accessDetail, bookingId, availabilityUrl, featuredImageAlt and locationRelations. Supporting structured authority: `src/data/locations.ts` (geo, map, subtitle, distanceTag), `src/data/gallery.json`, `src/data/siteCopy.json`, `src/inventory/groups.json` and `src/inventory/suggested-pairings.json` (recommendation-only per `src/inventory/AGENTS.md`). Property prose lives in `src/content/houses/` and `src/content/villa/`, with frontmatter limited to title, slug and summary by `src/content.config.ts`; the villa collection uses passthrough but its file adds no factual frontmatter.
- Facts intentionally narrative-only, with no structured counterpart: parking capacity, walking and driving distances, per-room bed placement, and individual appliances. Absence from the record is therefore not a contradiction, and the audit stage says so explicitly.
- Existing consistency coverage before Phase 10: `src/utils/validateInventory.ts` (house count, villa bookingId, unique slugs) and `tests/i18n-german-visible-language.test.mjs` (a German presentation must reuse the English source's numbers). Nothing compared the canonical record with the English property Markdown, so Phase 10 adds a small derived check for one property.
- New workspace `.agents/workspaces/property-content/CONTEXT.md` with three stages: `stages/01_content/CONTEXT.md` (bounded prose or practical-information change, never writes the factual record), `stages/02_factual_audit/CONTEXT.md` (read-only comparison with seven classifications, stops on a canonical conflict) and `stages/03_fact_correction/CONTEXT.md` (the only stage allowed to change a canonical fact, and only on an explicit authoritative correction). The third stage exists because the stage split is the permission boundary; folding it into Stage 01 would weaken Stage 01's absolute prohibition on editing `src/inventory/inventory.json`.
- Root routing: the single "Property facts or property-page factual audit" row became two rows that both route to the workspace, one for facts/corrections/audits and one for property page copy. The general editorial-copy row no longer claims property copy, and the architecture row now reads "with no implementation or content editing authorized yet" so a pure ownership decision still routes to `docs/architecture/`. `AGENTS.md` follows the same owner. The site-engineering workspace's non-owned row now names the property-content workspace while still naming the authority behind it.
- Recorded, not repaired: `.ai/prompts/website-editorial-system.md` still names its sources as inventory.json, locations.json and locationCopy.json, while the implemented files are `src/inventory/inventory.json`, `src/data/locations.ts` and `src/data/siteCopy.json`; and the legacy backup beside the factual record is unreferenced, as `docs/audits/repo-structure-audit-2026-06-09.md` already documents. The workspace tells an agent to report such drift rather than rewrite it, and forbids a data migration as a side effect.
- Proof property: Margarita, read and not modified. `tests/property-content-context-routing.test.mjs` adds 16 tests covering both root routes, the three workspace stages, the i18n/site-engineering/SEO boundaries, declared canonical authority, L3 resolution, narrow L4 selection, read-only audit, stop-on-conflict, the qualifier and limitation rules, the merge/deploy prohibition, and a live ICM audit. The last test derives Margarita's area, capacity, bedroom count, pool state, parking distance and every hard constraint from the record and asserts the English master neither contradicts nor drops them; no property fact is written as a literal in the test.
- `scripts/context/validate-icm.mjs` was again not extended: the generic validator already covers workspace routing, the five stage headings, L3/L4 declarations and dead references.
- Verified: `npm run context:audit` passed; `node --test` over the six control-plane test files (icm-context-routing, property-content, site-engineering, icm-topic-research, editorial-research, blog-orchestrator) 75/75 passed; `git diff --check` clean. Build and typecheck were not run and are not required: the diff is control-plane Markdown plus one new test file and one adapted assertion in the Phase 9 test.
- Not verified here: the full `node --test`. This session reached the repository through the desktop bridge, whose mount forbids `unlink` and whose Linux runtime cannot load the win32 `sharp` binary, so ten test files cannot run and two more fail only in teardown. The 28 files that do run pass 277/277. Re-run `node --test` natively on Windows; the expected count is the Phase 9 baseline of 398 plus the 16 new tests.

### 2026-08-21 - ICM Phase 9: site-engineering workspace (branch `chore/icm-phase9-site-engineering-workspace`)

- Objective: give non-i18n Astro/site implementation and build/runtime/type debugging one explicit ICM owner instead of loading architecture docs, operations notes and source files ad hoc. Branch base is the Phase 8 head `1ca81bbfdf121a26826fb94a9b4f7d9f5cb26a8a`; no runtime, route, style, inventory, translation, Cloudflare or configuration file was touched.
- New workspace `.agents/workspaces/site-engineering/CONTEXT.md` with two stages: `stages/01_implementation/CONTEXT.md` (approved bounded code change) and `stages/02_debugging/CONTEXT.md` (reproduce, classify source vs environment, smallest in-scope repair). No third validation stage: every stage contract already carries its own `## Verify` section, so a separate one would only duplicate the gate.
- The workspace classifies a request as A implementation, B debugging, C architecture decision (route to `docs/architecture/` and stop when no implementation is authorized) or D cross-domain (route to the owning workspace and stop).
- Two root routes moved into the workspace: "Build/debug/Windows cache or repeat-failure handling", previously pointed straight at `docs/operations/repeated-failures-playbook.md`, and "General Astro/UI implementation not covered above", previously pointed at unspecified `src/` files. The playbook is now stable Layer 3 inside the debugging stage rather than a root route target. `AGENTS.md` was updated to the same single owner. The architecture/source-of-truth row stays with `docs/architecture/` and is now explicitly the no-implementation-authorized route.
- Kept outside the workspace and named as such in it: i18n (`.agents/workspaces/i18n/CONTEXT.md`), editorial/research (`BLOG_ORCHESTRATOR.md`), property-fact authority (`docs/architecture/source-of-truth.md` with `src/inventory/inventory.json`), media authority (`docs/architecture/media-ownership.md`), and SEO/content-intelligence, social publication and deployment/Cloudflare operations, which have no ICM workspace yet and are a stop-and-report.
- Input discipline is contractual: Layer 3 rows are stable references loaded only when the task involves them; every Layer 4 row is narrowed to exact affected files, exact focused tests and the exact reproduction. Both stages state "Do not load all of `src/`, all of `tests/`, or all of `docs/`".
- Environment failures cannot authorize source edits: the debugging stage classifies source vs environment before editing, routes locks on generated output and stale caches through the playbook, and forbids editing source, renaming assets or committing generated output to clear a lock.
- New test `tests/site-engineering-context-routing.test.mjs`, 15 tests, covering both moved root routes, the preserved i18n/editorial/property-fact routes, the non-absorbed domains, one bounded stage at a time, the L3/L4 declarations (each L3 path is asserted to exist on disk), the no-whole-tree-loading rule, the reproduce-then-classify order, the environment-lock rule, unrelated diagnostics staying out of scope, the merge/deploy prohibition, and a live ICM audit run.
- `scripts/context/validate-icm.mjs` was deliberately not extended. The generic validator already enforces workspace routing, the five required stage headings, L3/L4 declarations and dead references, and it passes on the new workspace unchanged.
- Verified: `npm run context:audit` passed; `node --test tests/icm-context-routing.test.mjs tests/site-engineering-context-routing.test.mjs tests/icm-topic-research-context.test.mjs tests/editorial-research-context-routing.test.mjs tests/blog-orchestrator.test.mjs` 59/59 passed; `git diff --check` clean. Build and typecheck were not run and are not required: the change is control-plane Markdown plus one test file, with no runtime or build-sensitive source in the diff.
- Not verified here: the full `node --test` run. This session reached the repository through the desktop bridge, whose mount forbids `unlink`, so the five test files that build a temporary Astro config inside the repo (`duplicate-page-headings`, `i18n-german-route-pilot`, `i18n-german-visible-language`, `llms-txt`, `seo-bedroom-grammar`) fail on cleanup with `EPERM`, and the five that load `sharp` (`image-pipeline`, `homepage-image-delivery`, `process-blog-image.characterization`, `social-live-publisher`, `social-publisher`) fail because the installed `sharp` binary is win32. The other 27 test files pass there. Both causes are environment, not source; per the playbook no source was changed to work around them. Re-run `node --test` natively on Windows to confirm the 383/383 baseline.
...
### 2026-08-20 - German Vrouchas guide: faithful localization of the English master (PR #59)
