...
export async function reconcilePlatform({ ledger, platform, env, fetchImpl, persist, remoteId, confirmed = false, now = new Date() }) {
  ...
  if (String(remote.id) !== String(remotePostId)) throw new Error(`${platform} reconciliation could not confirm the remote publication.`);
  ...
  if (platform === "instagram") {
    const ownerId = remote.owner && typeof remote.owner === "object" && !Array.isArray(remote.owner) ? remote.owner.id : null;
    if (!/^\d+$/.test(String(ownerId ?? "")) || String(ownerId) !== String(values.META_IG_USER_ID)) throw new Error("Instagram remote media ownership cannot be established.");
  }
