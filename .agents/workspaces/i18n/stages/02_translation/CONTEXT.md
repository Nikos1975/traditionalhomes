...
One job: produce or validate a complete natural localization for an already-supported route family. Do not redesign multilingual infrastructure in this stage.
...
2. Confirm the target locale already has the required route family, generic path segments, wrapper, and shared renderer. For an existing dynamic route family, a new localized content id/slug may be declared in `src/i18n/route-map.ts` in this stage only when complete localized content is added in the same bounded change.
...
8. Do not leave a target-locale `routeMap.content` entry without the complete localized page that makes it real.

## Outputs

- target-locale content/resource changes for the approved scope;
- the matching content-availability declaration for an already-supported dynamic route family when required;
...
Stop if the required locale route family does not exist, a new generic route segment/wrapper/renderer is required, the English source is ambiguous, a factual correction is required, translation would duplicate structured inventory facts, unexpected files appear, English output changes, validation worsens, or merge/deploy/publication is requested without explicit approval.
